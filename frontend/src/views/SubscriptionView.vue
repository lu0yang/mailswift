...
const templateOptions = computed(() =>
  templates.value.filter(t => t.type === 'subscription').map(t => ({ label: t.name, value: t.id }))
)
...
const previewHtml = computed(() => {
  ...
  if (selectedSignatureId.value) {
    ...
    if (sig && sig.content) html += '<br>' + sig.content
  }
  return html
})
...
function renderTemplateContent(templateContent) {
  ...
  const lines = subs.map((s, i) => `${i + 1}. ${s.subscription_id} - ${s.subscription_name}`)
...
function substitutePlainMarkers(text) {
  ...
  text = text.replaceAll('{subscription_list}', subs.map((s, i) => `${i + 1}. ${s.subscription_id} - ${s.subscription_name}`).join('<br>') || '（无）')
  if (subs.length >= 1) {
...
const MARKERS = ['{subscription_list}', '{subscription_plural}', '{subscription_id}', '{subscription_name}', '{have_has}']
...
watch(
  ...
  () => {
    ...
    if (subject.value.trim() || recip.tags.value.length || recip.ccTags.value.length || body.value.trim() ||
        (formData.value.subscriptions || []).some(s => s.subscription_id || s.subscription_name)) {
...
async function handleSend() {
  ...
  try {
    const payload = {
      ...
      accounts: [],
      subscriptions: (formData.value.subscriptions || []).map(s => ({
    ...
    ;(formData.value.subscriptions || []).forEach(s => {
      if (s.subscription_id) history.addToHistory(FIELD_HISTORY_KEYS.subscription_id, s.subscription_id)
      if (s.subscription_name) history.addToHistory(FIELD_HISTORY_KEYS.subscription_name, s.subscription_name)
    })
    history.loadAll()
